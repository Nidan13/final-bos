...
func SetupMahasiswaRoutes(app *fiber.App) {
	...
	// PKKMB (Kencana)
	kencanaPerm := middleware.RequireAnyPermission("student.kencana.view", "student_kencana_view")
	kencanaGroup := api.Group("/kencana")
	kencanaGroup.Get("/progress", kencanaPerm, mahasiswa.GetProgress)
	kencanaGroup.Get("/kegiatan", kencanaPerm, mahasiswa.GetPkkmbKegiatan)
	kencanaGroup.Post("/check-in/:id", kencanaPerm, mahasiswa.CheckIn)
	kencanaGroup.Get("/sertifikat", kencanaPerm, mahasiswa.GetSertifikat)
	kencanaGroup.Post("/sertifikat/generate", kencanaPerm, mahasiswa.GenerateSertifikat)
	kencanaGroup.Get("/banding", kencanaPerm, mahasiswa.GetBandingList)
	kencanaGroup.Post("/banding", kencanaPerm, mahasiswa.SubmitBanding)
	kencanaGroup.Get("/kuis/:id/soal", kencanaPerm, mahasiswa.GetKuisSoal)
	kencanaGroup.Post("/kuis/:id/submit", kencanaPerm, mahasiswa.SubmitKuis)
