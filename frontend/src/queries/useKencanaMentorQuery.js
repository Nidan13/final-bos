...
export const useMentorAnnouncementsQuery = () => useQuery({
  queryKey: ['kencana-mentor', 'announcements'],
  queryFn: async () => unwrap(await api.get('/kencana-mentor/announcements')),
});
...
export const useMentorStudentAssignmentsQuery = (studentId) => useQuery({
  queryKey: ['kencana-mentor', 'student-assignments', studentId],
  queryFn: async () => asArray(unwrap(await api.get(`/kencana-mentor/students/${studentId}/assignments`))),
  enabled: !!studentId,
});
...
// ─── Bulk Scores ───
export const useMentorBulkScoresQuery = () => useQuery({
  queryKey: ['kencana-mentor', 'bulk-scores'],
  queryFn: async () => unwrap(await api.get('/kencana-mentor/bulk-scores')),
});

export const useMentorSubmitBulkScoresMutation = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (payload) => unwrap(await api.post('/kencana-mentor/bulk-scores', payload)),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['kencana-mentor', 'bulk-scores'] });
      qc.invalidateQueries({ queryKey: ['kencana-mentor', 'students'] });
      qc.invalidateQueries({ queryKey: ['kencana-mentor', 'dashboard'] });
    },
  });
...
export const useMentorReviewHandbookMutation = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ studentId, action, feedback }) => 
      unwrap(await api.post(`/kencana-mentor/students/${studentId}/handbook/review`, { action, feedback })),
    onSuccess: (_, vars) => {
...
export const useMentorAbsenceRequestsQuery = () => useQuery({
  queryKey: ['kencana-mentor', 'absence-requests'],
  queryFn: async () => asArray(unwrap(await api.get('/kencana-mentor/absence-requests'))),
});

export const useMentorRespondAbsenceRequestMutation = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, action }) => unwrap(await api.post(`/kencana-mentor/absence-requests/${id}/respond`, { action })),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['kencana-mentor', 'absence-requests'] });
    },
  });
};
