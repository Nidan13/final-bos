...
export default function TenagaKesehatanDashboardPage() {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const [tkRes, bkRes, mrRes, periodsRes, facRes, prodiRes] = await Promise.all([
      ...
      if (tkRes.status === 'success') {
        ...
        setData(resData.length > 0 ? resData : [
          { id: 1, nama: 'Dr. Ahmad', spesialisasi: 'Dokter Umum' },
          { id: 2, nama: 'Ns. Siti', spesialisasi: 'Perawat' }
        ])
      }
      if (bkRes.status === 'success') {
        ...
        setBookings(processedData.length > 0 ? processedData : [
          { status: 'Dikonfirmasi', jadwal: { tanggal: new Date().toISOString(), tipe_layanan: 'Pemeriksaan Umum', jam_mulai: '08:00', jam_selesai: '09:00' }, mahasiswa: { nama: 'Budi Santoso', nim: '10123456', fakultas: { Nama: 'Fakultas Ilmu Komputer' }, program_studi: { nama: 'Informatika' } }, tenaga_kesehatan: { nama: 'Dr. Ahmad', spesialisasi: 'Dokter Umum' } },
          { status: 'Selesai', jadwal: { tanggal: new Date().toISOString(), tipe_layanan: 'Konsultasi Gizi', jam_mulai: '10:00', jam_selesai: '11:00' }, mahasiswa: { nama: 'Siti Aminah', nim: '10123457', fakultas: { Nama: 'Fakultas Ekonomi' }, program_studi: { nama: 'Manajemen' } }, tenaga_kesehatan: { nama: 'Ns. Siti', spesialisasi: 'Perawat' } }
        ])
      }
  ...
  const filteredBookings = useMemo(() => {
    return bookings.filter(b => {
      ...
      if (activeFilters.periodId !== 'all') {
        ...
        if (selectedPeriod) {
          const dateStr = b.jadwal?.tanggal || b.Jadwal?.Tanggal || b.tanggal || b.Tanggal
  ...
  const filteredMedicalRecords = useMemo(() => {
    return medicalRecords.filter(mr => {
      ...
      if (activeFilters.periodId !== 'all') {
        const selectedPeriod = periods.find(p => String(p.id || p.ID) === String(activeFilters.periodId))
        if (selectedPeriod) {
          const dateStr = mr.tanggal_dibuat || mr.TanggalDibuat || mr.CreatedAt || mr.created_at || mr.tanggal_pemeriksaan || mr.TanggalPemeriksaan
          if (dateStr && !isDateInPeriod(dateStr, selectedPeriod)) return false
        }
      }

      return true
    })
  }, [medicalRecords, activeFilters, periods])

  const getTodayBookingsCount = () => {
    return filteredBookings.filter(b => {
      const d = b.jadwal?.tanggal || b.Jadwal?.Tanggal || b.tanggal || b.Tanggal
      if (!d) return false
      const bd = new Date(d)
  ...
  const stats = useMemo(() => {
    ...
    const selesaiBookings = filteredBookings.filter(b => {
      ...
      return statusLower === 'selesai' || statusLower === 'completed' || statusLower === 'disetujui' || statusLower === 'dikonfirmasi' || statusLower === 'confirmed'
    }).length
    const tingkatPenyelesaian = totalBookings > 0 ? ((selesaiBookings / totalBookings) * 100).toFixed(0) + '%' : '0%'

    const activeMedics = data.filter(p => p.is_aktif !== false).length
    const rerataBebanKerja = activeMedics > 0 ? (totalBookings / activeMedics).toFixed(1) : '0'
  ...
  const tenagaKesehatanWorkload = useMemo(() => {
    const counts = {}
    filteredBookings.forEach(b => {
      const tkId = b.tenaga_kesehatan_id || b.TenagaKesehatanID || b.tenaga_kesehatan?.id || b.tenaga_kesehatan?.ID || b.TenagaKesehatan?.id || b.TenagaKesehatan?.ID || ''
      if (tkId) {
        counts[tkId] = (counts[tkId] || 0) + 1
      }
    })
    return counts
  }, [filteredBookings])
  ...
  const bookingColumns = [
    ...
    },
    {
      ...
      render: (v, row) => (
        <div className="flex flex-col py-1 font-inter">
          <span className="font-bold text-[var(--theme-text)] text-xs">{(row.tenaga_kesehatan?.nama || row.TenagaKesehatan?.Nama) || '—'}</span>
          <span className="text-[9px] text-[var(--theme-text-muted)] font-bold uppercase tracking-wider">{(row.tenaga_kesehatan?.spesialisasi || row.TenagaKesehatan?.Spesialisasi) || '—'}</span>
        </div>
      )
    },
    {
      ...
      render: (v, row) => {
        const dateStr = row.jadwal?.tanggal || row.Jadwal?.Tanggal || row.tanggal || row.Tanggal;
        const formattedDate = dateStr ? new Date(dateStr).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }) : '—';
        const jamMulai = row.jadwal?.jam_mulai || row.Jadwal?.JamMulai || row.jam_mulai || row.JamMulai || '—';
        const jamSelesai = row.jadwal?.jam_selesai || row.Jadwal?.JamSelesai || row.jam_selesai || row.JamSelesai || '—';
        return (
          <div className="flex flex-col items-center py-1 font-inter">
            ...
            <span className="text-[10px] text-[var(--theme-text-muted)] font-semibold">{jamMulai !== '—' && jamSelesai !== '—' ? `${jamMulai} - ${jamSelesai}` : jamMulai}</span>
          </div>
        )
      }
    },
    {
      ...
      render: (v, row) => {
        const t = row.jadwal?.tipe_layanan || row.Jadwal?.TipeLayanan || row.tipe_layanan || 'Pemeriksaan Umum'
        return <span className="text-[9px] font-bold text-[var(--theme-primary)] bg-[var(--theme-primary-light)] px-2.5 py-0.5 rounded-lg border border-[var(--theme-primary)]/10 whitespace-nowrap">{t}</span>
      }
    },
    {
      key: 'status',
      ...
      render: v => {
        ...
        } else if (statusLower === 'ditolak' || statusLower === 'dibatalkan' || statusLower === 'batal') {
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5">
            ...
            <PrimaryStatsCard
              title="Rekam Medis"
              value={<>{filteredMedicalRecords.length} <span className="text-sm font-bold text-slate-400">Data</span></>}
          ...
          <div className="bg-[var(--theme-surface)] rounded-2xl border border-[var(--theme-border)] shadow-sm overflow-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-[var(--theme-border)]">
              <Link to="/app/dashboard/tenagakes/list" className="p-6 hover:bg-[var(--theme-bg)] transition-all duration-300 group text-left flex flex-col justify-between">
                <div>
                  ...
                  <h3 className="font-bold text-[var(--theme-text)] text-sm mb-1 group-hover:text-primary transition-colors font-headline">Direktori Tenaga Medis</h3>
                  <p className="text-xs text-[var(--theme-text-muted)] font-medium leading-relaxed">Kelola akun, penugasan, dan status aktif tenaga medis kesehatan.</p>
                </div>
              </Link>

              <Link to="/app/dashboard/tenagakes/bookings" className="p-6 hover:bg-[var(--theme-bg)] transition-all duration-300 group text-left flex flex-col justify-between">
                <div>
                  ...
                  <h3 className="font-bold text-[var(--theme-text)] text-sm mb-1 group-hover:text-info transition-colors font-headline">Booking Klinik</h3>
                  <p className="text-xs text-[var(--theme-text-muted)] font-medium leading-relaxed">Pantau agenda jadwal dan verifikasi status janji temu mahasiswa di klinik.</p>
                </div>
              </Link>

              <Link to="/app/dashboard/tenagakes/medical-records" className="p-6 hover:bg-[var(--theme-bg)] transition-all duration-300 group text-left flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-xs text-[var(--theme-text-muted)] font-medium leading-relaxed">Akses log konsultasi klinis, riwayat resep, dan hasil anamnesis mahasiswa.</p>
                </div>
              </Link>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 animate-in fade-in duration-500">
            ...
            <div className="lg:col-span-2">
              <PageCard className="h-full">
                <PageCardHeader title="Tren Kunjungan Klinik" icon="show_chart" />
            ...
            {/* Pie Chart: Layanan Kesehatan Terpopuler */}
            <div className="lg:col-span-1">
              <PageCard className="h-full flex flex-col justify-between">
                <PageCardHeader title="Layanan Terpopuler" icon="pie_chart" />
                <div className="h-[140px] w-full flex items-center justify-center mt-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in duration-500">
            {/* Left Column (span 2): Toggleable Card */}
            <div className="lg:col-span-2">
              <PageCard className="h-full flex flex-col">
                <PageCardHeader 
                  title={insightsTab === 'list' ? "Daftar Tenaga Medis & Beban Kerja" : "Sebaran Spesialisasi Layanan"} 
                ...
                {insightsTab === 'list' ? (
                  <div className="mt-4 space-y-3 flex-1 max-h-[400px] overflow-y-auto no-scrollbar pr-2">
                    {data.length > 0 ? (
                      data.map((p) => {
                        const tkId = p.id || p.ID;
                        const workload = tenagaKesehatanWorkload[tkId] || 0;
                        const isActive = p.is_aktif ?? p.IsAktif !== false; // Active by default
                        return (
                          <div 
                            key={tkId} 
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="text-[10px] text-[var(--theme-text-muted)] font-medium mt-0.5">{p.spesialisasi || 'Dokter Umum'}</p>
                      ...
                      <div className="py-12 text-center text-xs text-[var(--theme-text-muted)] italic">
                        Tidak ada data tenaga medis terdaftar
                      </div>
                    )}
                  </div>
                ) : (
            ...
            <div className="lg:col-span-1">
              <PageCard className="h-full flex flex-col">
                <PageCardHeader title="Pasien Teraktif" icon="group" />
          ...
          <div className="mt-6 mb-6 animate-in fade-in duration-300">
            <DataTable
              title="Riwayat & Agenda Kunjungan Medis"
              subtitle="Menampilkan data kunjungan klinik seluruh tenaga medis sesuai filter aktif."
              columns={bookingColumns}
              data={filteredBookings}
              loading={loading}
              searchPlaceholder="Cari Nama Pasien, NIM, atau Layanan..."
            />
          </div>
        </div>
      )}
